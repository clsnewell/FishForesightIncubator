...
4. I expand plus count data
There are 2 approaches: Poisson and proportional.
...
Proportional: I take each tow, species combination and calculate the proportion of age classes represented in the tow for the species. I then apply that proportion to the plus counted fish. This results in 9 fish accidentally deleted and 1 artificially created due to the math of the rounding. Overall, not terribly meaningful impact. But also imperfect.
...
### Investigating NA's in dataset (DOES NOT REMOVE THEM FROM SMFS_Integrate)
```{r}
...
#write.csv(GobyIdNeed, "C:/Users/cnewe/OneDrive/Documents/Incubator/Code/FishForesightIncubator/Data/MissingGobyIDs.csv")
...
SMFS_Integrate_NoNAFish<-SMFS_Integrate %>% filter(!gensp == "NA NA")
```
...
```{r}
SMFS_Integrate_NoNAFish_Expand <- SMFS_Integrate_NoNAFish %>%
...
SMFS_Integrate_NoNAFish_Expand$Count<-1 #assigns value of 1
unique(SMFS_Integrate_NoNAFish_Expand$Class)
nrow(SMFS_Integrate_NoNAFish_Expand)
SMFS_Integrate_NoNAFish_Expand <- SMFS_Integrate_NoNAFish_Expand %>%
  dplyr::mutate(FishID = row_number()) #df becomes size 0. Hopefully that is okay?
PlusCountDF<-SMFS_Integrate_NoNAFish %>%
  dplyr::filter(!StandardLength > 0)
PlusCountDF <- PlusCountDF %>%
  dplyr::mutate(FishID = NA)
SMFS_Integrate_NoNAFish_Expand2<-rbind(SMFS_Integrate_NoNAFish_Expand, PlusCountDF)
unique(PlusCountDF$Class)

OtherPlus<-PlusCountDF %>% filter(Class %in% c("Malacostraca", "Crustacea", "Bivalvia", "many", "Hydrozoa", "N/A",  "Insecta", "Angiosperms",  "NA"))

#Save New DF
#write.csv(SMFS_Integrate_NoNAFish_Expand2, "C:/Users/cnewe/OneDrive/Documents/Incubator/Code/FishForesightIncubator/Data/SMFS_OneFishOneRow.csv")
...
unique(SMFS_Integrate_NoNAFish_Expand$OrganismCode) #77 including non-fish
SMFS_Integrate_NoNAFish_t<- SMFS_Integrate_NoNAFish_Expand %>% 
  dplyr::filter(Class %in% c("Osteichthyes","Cephalaspidomorphi")) 
unique(SMFS_Integrate_NoNAFish_t$OrganismCode) #48 fishes. 
...
AgedFish<-SMFS_Integrate_NoNAFish_Expand %>% filter(OrganismCode%in%c("LFS", "SB", "TP", "STAG", "SF", "CP", "DS", "ISS", "WCF", "TFS", "ST", "STBK", "SCP", "SKR", "BF", "BLB", "CCF", "CS", "HCH", "MQF", "NAC", "PH", "RWK", "SPM", "WAK", "WC", "WS", "YFG", "ASH", "SG", "SKG", "BC", "GF")) 
...
BringEmBack<-SMFS_Integrate_NoNAFish_Expand %>% filter(!OrganismCode %in% c("LFS", "SB", "TP", "STAG", "SF", "CP", "DS", "ISS", "WCF", "TFS", "ST", "STBK", "SCP", "SKR", "BF", "BLB", "CCF", "CS", "HCH", "MQF", "NAC", "PH", "RWK", "SPM", "WAK", "WC", "WS", "YFG", "ASH", "SG", "SKG", "BC", "GF")) %>% mutate(Max = NA, Min = NA, AgeClass = "N/A")
...
PlusCountDF<-PlusCountDF %>% filter(Class %in% c("Osteichthyes","Cephalaspidomorphi")) #Technically we don't have any plus count Cephalaspidomorphi but in case they appear in the future this code is needed.
...
PlusAssigned$PlusID<-1:9523 #I need to track these fish. Giving them each a number for simplicity.
...
joindf<-SMFS_Integrate_NoNAFish %>% select(OrganismCode, SampleRowID, Month, gensp)
...
Aged_Plus$id_sp <- paste0(Aged_Plus$SampleRowID, "_", Aged_Plus$gensp)
...
#### Comparing plus assignments to comments
```{r}
...
Comparison<-Aged_Plus_Final$id_sp
Comparison

ShouldYOY<-Aged_Plus_Final %>% filter(id_sp %in% YoyList)
...
Aged_Plus_Final$AgeClass[Aged_Plus_Final$PlusID %in% c(1099, 1109, 1131, 1140, 1144, 1174, 4708, 8225, 8226)]<-"Age-0" #fixing age class
Aged_Plus_Final$sl[Aged_Plus_Final$PlusID %in% c(1099, 1109, 1131, 1140, 1144, 1174, 4708, 8225, 8226)]<-40 #fixing sl. 40 should keep all of these fish squarely in the yoy category. 
ShouldYOY2<-Aged_Plus_Final %>% filter(id_sp %in% YoyList)
ShouldYOYButNotYOY2<-ShouldYOY2 %>% filter(!AgeClass%in%"Age-0")#Done

#Join these Age_Plus_Final back to whole dataset while ensuring we have kept track of these changes.

#There are some fish with no length assignment because they have no measured fish to be based off of. They were one-offs that length measurements were missed on. Levi wants me to deal with them by basing their lengths on lengths of the day.

unique(PlusCountDF$AgeClassforUnmeasuredFish) #this is a pointless column.
...
When I put these all back together, I should have the same number of fish that I started with...
```{r}
glimpse(Aged_Plus_Final) #10 columns. 9523 observations
glimpse(OtherPlus) #49 columns. 20,386 observations
glimpse(DataWithAges) #49 columns.69998 observations. 
```
To accomplish my goal of stitching everything back together, I will aim to get all of the columns into agreement and then rbind them.

1. I need to add id_sp to OtherPlus.
2. I need to use sample row ID to get the sampling info placed back in for the Aged_Plus_Final dataframe.
3. Change the name of the Aged_Plus_Final "sl" column to "StandardLength" to match the other dataframes.
4. Add "type" column to DataWithAges and OtherPlus. Give values indicating they are unchanged/unmanipulated
5. Add "PlusID" column to DataWithAges and OtherPlus. Give NA as value
6. set and order of columns need to match I think. Then rbind and check we've got the right number of fish.

```{r}
#1
OtherPlus$id_sp <- paste0(OtherPlus$SampleRowID, "_", OtherPlus$gensp) 

#2
Samples<-SMFS_Integrate_NoNAFish %>% select(SampleRowID, StationCode, SampleDate, SampleTime, WaterTemperature, Salinity, DO, PctSaturation, Secchi, SpecificConductance, TideCode, UserName, ElecCond, TrawlRowID, TowDuration, TrawlComments, Year, Month, WaterYear, OrganismRowID, Native, Resident, CommonName, Phylum, Class, Order, Family, Genus,Species, Comments, gensp) %>% distinct()

Aged_Plus_Full<-left_join(Aged_Plus_Final, Samples)
glimpse(Aged_Plus_Full) #9523 rows, 38 columns

#3
Aged_Plus_Full$StandardLength<-Aged_Plus_Full$sl

#4
DataWithAges$type<-"Measured"
OtherPlus$type<-"No measure or manipulation"

#5
DataWithAges$PlusID<-NA
OtherPlus$PlusID<-NA

Aged_Plus_Full$FishID<-paste0(Aged_Plus_Full$PlusID, "_", "PLUS")
OtherPlus$FishID<-"Not Fish"
DataWithAges$FishID<-paste0(DataWithAges$FishID, "_", "Measured")

#6
Aged_Plus_Full$Volume<-NA
OtherPlus$AgeClass<-NA
glimpse(Aged_Plus_Full) #40 columns. 9523 observations
glimpse(OtherPlus) #51 columns. 20,386 observations
glimpse(DataWithAges) #51 columns.69998 observations.


OtherPlus_ToBind<-OtherPlus %>% select(FishID, id_sp, SampleRowID, gensp, OrganismCode, StandardLength, AgeClass, Count, Volume, type, StationCode, SampleDate, SampleTime, WaterTemperature, Salinity, DO, PctSaturation, Secchi, SpecificConductance, ElecCond, TideCode,  UserName, TrawlRowID, TowDuration, TrawlComments, Year, Month, WaterYear, OrganismRowID, Native, Resident, CommonName, Phylum, Class, Order, Family, Genus,Species, Comments, PlusID)

DataWithAges_ToBind<-DataWithAges %>%  select(FishID, id_sp, SampleRowID, gensp, OrganismCode, StandardLength, AgeClass, Count, Volume, type, StationCode, SampleDate, SampleTime, WaterTemperature, Salinity, DO, PctSaturation, Secchi, SpecificConductance, ElecCond, TideCode,  UserName, TrawlRowID, TowDuration, TrawlComments, Year, Month, WaterYear, OrganismRowID, Native, Resident, CommonName, Phylum, Class, Order, Family, Genus,Species, Comments, PlusID)

Aged_Plus_Full_ToBind<-Aged_Plus_Full %>%  select(FishID, id_sp, SampleRowID, gensp, OrganismCode, StandardLength, AgeClass, Count, Volume, type, StationCode, SampleDate, SampleTime, WaterTemperature, Salinity, DO, PctSaturation, Secchi, SpecificConductance, ElecCond, TideCode,  UserName, TrawlRowID, TowDuration, TrawlComments, Year, Month, WaterYear, OrganismRowID, Native, Resident, CommonName, Phylum, Class, Order, Family, Genus,Species, Comments, PlusID)
  
FullyReadyDataWithAge<-bind_rows(DataWithAges_ToBind, Aged_Plus_Full_ToBind, OtherPlus_ToBind)
sum(FullyReadyDataWithAge$Count)
sum(SMFS_Integrate_NoNAFish$Count) #Checks out!
```

### Proportional assignments by age class to plus counts (Not used)
```{r}

SMFS_Integrate_NoNAFish_ExpandAge <- DataWithAges %>%
  filter(StandardLength > 0) %>% #only keeping measured fish
  uncount(Count, .remove = FALSE) #keeps number of rows that corresponds to the Count value
SMFS_Integrate_NoNAFish_ExpandAge$Count<-1 #assigns value of 1

PlusCountDFAge<-DataWithAges %>%
  filter(!StandardLength > 0)
SMFS_Integrate_NoNAFish_Expand2<-rbind(SMFS_Integrate_NoNAFish_ExpandAge, PlusCountDFAge)
...
#bring back the others later: "Malacostraca" "Crustacea"    "Bivalvia"    [4] "many"         "Hydrozoa"     "N/A"   "Insecta"      "Angiosperms" [10] "NA"    
unique(SMFS_Integrate_NoNAFish_ExpandAge$AgeClass)

exp_table <- 
  # lengths_df  %>%                       # formatted raw catch data
  SMFS_Integrate_NoNAFish_ExpandAge %>%
...
#Step 1: Get structure of t to match SMFS_Integrate_NoNAFish_Expand. 1 row = 1 fish 
View(DataWithAges)
...
Ages<-SMFS_AgesBySizeMo %>% select(Class, OrganismCode) %>% rename(AgeClass = Class) %>% filter(AgeClass %in% c("Age-0",  "Age-1+", "Age-1",  "Age-2+")) %>%   distinct()

#Now for every sampling event, repeat all of the species x age combinations. Make a df of the sampling events.
SamplingEvents<-SMFS_Integrate %>% select(SampleRowID, StationCode, SampleDate, SampleTime, WaterTemperature, Salinity, DO, PctSaturation, Secchi, SpecificConductance, TideCode, UserName, ElecCond, TrawlRowID, TowDuration, TrawlComments, Year, Month, WaterYear) %>% distinct()
...
has_ageclass_info <- FullyReadyDataWithAge %>% filter(AgeClass %in% c("Age-0",  "Age-1+", "Age-1",  "Age-2+"))

no_ageclass_info <- FullyReadyDataWithAge %>% filter(AgeClass %in% c("N/A",  NA))
...
Massive_Codes<-Massive %>% select(!c("Native", "Resident", "CommonName", "Phylum", "Class", "Order", "Family", "Genus", "Species", "Comments"))

Massive_Codes<-left_join(Massive_Codes, SMFS_OrganismsLookUp, by="OrganismCode")
Massive_Codes<-mutate(Massive_Codes, gensp = paste(Genus, Species, sep=" "), id_sp = paste(SampleRowID, gensp, sep="_")) %>% select(!c("NonAgedOrganisms", "OrganismRowID.y", "OrganismRowID.x"))
unique(Massive_Codes$OrganismCode)

#Add in species info that is missing


#SMFS_OTR_Thesis_AgeClassWithZeros<-FullyReadyDataWithAge %>% 
#  complete(nesting(FishID,
#                   id_sp,
#                   SampleRowID, 
#                   gensp,
#                   StandardLength,
...
```{r}
#Save New DF
write.csv(Massive_Codes, "C:/Users/cnewe/OneDrive/Documents/Incubator/Code/FishForesightIncubator/Data/SMFS_WithAbsences.csv")
