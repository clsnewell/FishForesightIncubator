...
Plot_Water<-SCPData %>% mutate(water_temp_bin = as.factor( round(WaterTemperature * 2) / 2))  %>% mutate(water_temp_bin = as.numeric(water_temp_bin))  %>% 
  group_by(water_temp_bin, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=water_temp_bin, y=CPUE, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs( x="Water Temperature (C)", caption = "Water temperatures rounded to 0.5") + theme(axis.text.x = element_text(angle = 45, hjust=1))+ guides(fill="none")


Plot_DO<-SCPData %>% mutate(do_bin = as.factor(round(DO * 2) / 2))  %>% mutate(do_bin = as.numeric(do_bin))  %>%  group_by(do_bin, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
...
Plot_Salinity<-SCPData %>% mutate(salinity_bin = as.factor(round(Salinity * 2) / 2))  %>%  mutate(salinity_bin = as.numeric(salinity_bin)) %>%  group_by(salinity_bin, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=salinity_bin, y=CPUE, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Salinity (ppt)", caption = "Salinity  rounded to 0.5.") + guides(fill="none")
