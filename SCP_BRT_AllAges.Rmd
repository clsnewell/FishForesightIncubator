...
Plot_DO<-SCPData %>% mutate(do_bin = as.factor(round(DO * 2) / 2))  %>% group_by(do_bin) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=do_bin, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "Prickly sculpin & dissolved oxygen SMFS 2011-2023", y ="Catch per minute trawl", x="Dissolved Oxygen (mg/L)", caption = "Dissolved oxygen rounded to 0.5 then grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")
...
Plot_Secchi<-SCPData %>% group_by(Secchi) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=Secchi, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "Prickly sculpin & dissolved oxygen SMFS 2011-2023", y ="Catch per minute trawl", x="Secchi (cm)", caption = "Secchi grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")
...
Plot_Salinity<-SCPData %>% mutate(salinity_bin = as.factor(round(Salinity * 2) / 2))  %>% group_by(salinity_bin) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=salinity_bin, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "Prickly sculpin & dissolved oxygen SMFS 2011-2023", y ="Catch per minute trawl", x="Salinity", caption = "Salinity  rounded to 0.5 then grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")
...
#Seasonality
SCPData %>% dplyr::summarize(TotalCount=sum(Count), sd_Count=sd(Count), n=n(), PercentageOfPresence=100*(TotalCount/n)) #Total presence = 1971 of 5471. 36.03% presence of samples.
SCPData %>%  group_by(Month) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=Month, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "Prickly sculpin by month SMFS 2011-2023", y ="Catch per minute trawl", x="Month", caption = "Month grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")
...
SCPData %>% group_by(StationCode) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=StationCode, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "Prickly sculpin by station SMFS 2011-2023", y ="Catch per minute trawl", x="Station Code", caption = "Station code grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")+ theme(axis.text.x = element_text(angle = 45, hjust=1))
...
### gbm.step
```{r}
system.time( brt3 <- dismo::gbm.step(data=SCPData_All, 
...
# save the model
saveRDS(brt3, here("data", "AllSCP_count_brt_gbm_step.rds"))
```
...
Preds<-predict(object = brt2, newdata = SCPData_Positive_Test, type = "response")

#PredGrid<-gbm.predict.grids(brt2, SCPData_Positive_Test, want.grids=F, n.trees=brt2$gbm.call$best.trees, type="response")
PredBinaries<-as.factor(ifelse(Preds>0.5,1,0))
