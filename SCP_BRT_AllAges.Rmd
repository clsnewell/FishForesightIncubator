...
LFS %>% ggplot(aes(x=StandardLength, fill=Type)) + geom_histogram() + ggtitle("Length distribution of Longfin smelt 2011-2023") + theme(text=element_text(size=20))# + 

PresAbs %>% mutate(Type= ifelse(is.na(FishID), "Plus", "Measured"), length_bin = as.factor(round(StandardLength / 2) * 2)) %>% filter(gensp %in% "Cottus asper", !AgeClass=="NA") %>%  %>%  ggplot(aes(x=StandardLength,y=Count, fill=AgeClass)) + geom_bar(stat="identity")+ ggtitle("Length distribution of prickly sculpin 2011-2023") +  theme_bw(base_size = 26) #6044 zeros removed to make plot.

PresAbs %>% mutate(Type= ifelse(is.na(FishID), "Plus", "Measured"), length_bin = as.factor(round(StandardLength / 2) * 2)) %>% filter(gensp %in% "Spirinchus thaleichthys") %>% ggplot(aes(x=StandardLength,y=Count, fill=Type)) + geom_bar(stat="identity")+ ggtitle("Length distribution of Longfin smelt 2011-2023") + theme(text=element_text(size=20))
