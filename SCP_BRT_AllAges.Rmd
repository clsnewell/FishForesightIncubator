...
Tester<-SCPData %>% mutate(water_temp_bin = as.factor( round(WaterTemperature * 2) / 2))  %>% mutate(water_temp_bin = as.numeric(water_temp_bin))  %>% 
  group_by(water_temp_bin, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow)

Plot_Water<-SCPData %>% mutate(water_temp_bin = as.factor(round(WaterTemperature)))  %>% mutate(water_temp_bin = as.numeric(water_temp_bin))  %>% 
  group_by(water_temp_bin, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=water_temp_bin, y=CPUE, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs( x="Water Temperature (C)") + guides(fill="none")


Plot_DO<-SCPData %>% mutate(do_bin = as.factor(round(DO)))  %>% mutate(do_bin = as.numeric(do_bin))  %>%  group_by(do_bin, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
...
Plot_Secchi<-SCPData %>% group_by(Secchi, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ...
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Secchi (cm)")+ guides(fill="none")
...
Plot_Salinity<-SCPData %>% mutate(salinity_bin = as.factor(round(Salinity)))  %>%  mutate(salinity_bin = as.numeric(salinity_bin)) %>%  group_by(salinity_bin, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=salinity_bin, y=CPUE, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Salinity (ppt)") + guides(fill="none")
...
PresAbs %>% filter(gensp %in% "Cottus asper", !AgeClass=="NA") %>%   ggplot(aes(x=StandardLength,y=Count, fill=AgeClass)) + geom_bar(stat="identity")+ ggtitle("Length distribution of prickly sculpin 2011-2023") + xlab("Standard length (mm)")+ theme_bw(base_size = 26) #6044 zeros removed to make plot.


#Plot lengths by plus counts 
#PresAbs %>% mutate(Type= ifelse(is.na(FishID), "Plus", "Measured"), sl_bin = as.numeric(round(StandardLength))) %>% filter(gensp %in% "Cottus asper") %>% ggplot(aes(x=sl_bin,y=Count, fill=Type)) + geom_bar(stat="identity")+ ggtitle("Length distribution of prickly sculpin 2011-2023") +theme_bw(base_size = 20)+xlab("Standard length (mm)")

PresAbs %>% mutate(Type= ifelse(is.na(FishID), "Plus", "Measured")) %>% filter(gensp %in% "Cottus asper") %>% ggplot(aes(x=StandardLength,y=Count, fill=Type)) + geom_bar(stat="identity")+ ggtitle("Length distribution of prickly sculpin 2011-2023") +theme_bw(base_size = 20)+xlab("Standard length (mm)")


PresAbs$Month<-as.factor(PresAbs$Month)

PresAbs %>% filter(gensp %in% "Cottus asper") %>% group_by(Month) %>% dplyr::summarize(TotalCount=sum(Count), TotalTrawlTime=sum(TowDuration), CPUE= TotalCount/TotalTrawlTime) %>% ggplot(aes(x=Month, y=CPUE)) +geom_col()+theme_bw(base_size = 26)+ylab("Catch per minute trawl") +xlab("Month (summed 2011-2023)") + ggtitle("Prickly sculpin CPUE Seasonality")
```
...
Plot_Salinity<-PricklySculpinData_AllAges %>% ggplot(aes(y=PresAbs, x=Salinity)) + geom_violin(fill = "lightseagreen")+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw(base_size = 18)+ ylab("") + xlab("Salinity (ppt)") #+ facet_wrap(~Year)
...
#plots
dismo::gbm.plot(SCPbrt, n.plots = 4, write.title= F, rug = T, smooth = TRUE, plot.layout=c(1,4), common.scale = T, y.label = "Fitted Value") #Makes the partial dependence plots.

dismo::gbm.plot(scp_brt_allcount, n.plots = 4, write.title= F, rug = T, smooth = TRUE, plot.layout=c(1,4), common.scale = T, y.label = "Fitted Value") #Makes the partial dependence plots.

dismo::gbm.plot(SCP_BRT_PosCount, n.plots = 4, write.title= F, rug = T, smooth = TRUE, plot.layout=c(1,4), common.scale = T, y.label = "Fitted Value") #Makes the partial dependence plots.

#library(ggplotify)
#library(cowplot)

# Capture each set of plots as ggplot objects
#plot1 <- as.ggplot(~ dismo::gbm.plot(SCPbrt, n.plots = 4, write.title = F, rug = T, smooth = TRUE, plot.layout = c(1, 4), common.scale = T, y.label = "Fitted Value"))
#plot2 <- as.ggplot(~ dismo::gbm.plot(scp_brt_allcount, n.plots = 4, write.title = F, rug = T, smooth = TRUE, plot.layout = c(1, 4), common.scale = T, y.label = "Fitted Value"))
#plot3 <- as.ggplot(~ dismo::gbm.plot(SCP_BRT_PosCount, n.plots = 4, write.title = F, rug = T, smooth = TRUE, plot.layout = c(1, 4), common.scale = T, y.label = "Fitted Value"))

# Arrange the plots vertically
#cowplot::plot_grid(plot1, plot2, plot3, ncol = 1, rel_heights = c(1, 1, 1.9))


# Arrange the plots with alignment and space
#cowplot::plot_grid(plot1, plot2, plot3, ncol = 1, align = "v", axis = "lr", rel_heights = c(1,1,1), vjust = 2)
...
WaterTempDenisty<-ggplot(PricklySculpinData_AllAgesPosCount, aes(WaterTemperature))+geom_area(stat="bin")+ theme_bw()

SalinityDenisty<-ggplot(PricklySculpinData_AllAgesPosCount, aes(Salinity))+geom_area(stat="bin")+ theme_bw()

DODenisty<-ggplot(PricklySculpinData_AllAgesPosCount, aes(DO))+geom_area(stat="bin")+ theme_bw()

SecchiDenisty<-ggplot(PricklySculpinData_AllAgesPosCount, aes(Secchi))+geom_area(stat="bin")+ theme_bw()
...
```{r}
SCPData_Positive<-PresAbs %>% filter(gensp %in% "Cottus asper", Binary == 1) %>% group_by(SampleRowID, TowDuration, AgeClass, WaterTemperature, Salinity, Secchi, DO) %>% summarize(Count=sum(Count))
SCPData_Positive<-as.data.frame(SCPData_Positive)
summary(SCPData_Positive)
length(unique(SCPData_Positive$SampleRowID))


Plot_Water<-SCPData_Positive %>% mutate(water_temp_bin = as.factor(round(WaterTemperature)))  %>% mutate(water_temp_bin = as.numeric(water_temp_bin))  %>% 
  group_by(water_temp_bin, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=water_temp_bin, y=CPUE, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs( x="Water Temperature (C)") + guides(fill="none")
...
Plot_DO<-SCPData_Positive %>% mutate(do_bin = as.factor(round(DO)))  %>% mutate(do_bin = as.numeric(do_bin))  %>%  group_by(do_bin, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=do_bin, y=CPUE, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Dissolved Oxygen (mg/L)")
...
Plot_Secchi<-SCPData_Positive %>% group_by(Secchi, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=Secchi, y=CPUE, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Secchi (cm)")+ guides(fill="none")
#SCPData %>% ggplot(aes(y=Count, x=Secchi)) + stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw()+ ylab("Count") + xlab("Secchi (cm)") #+ facet_wrap(~Year)
...
Plot_Salinity<-SCPData_Positive %>% mutate(salinity_bin = as.factor(round(Salinity)))  %>%  mutate(salinity_bin = as.numeric(salinity_bin)) %>%  group_by(salinity_bin, AgeClass) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=salinity_bin, y=CPUE, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Salinity (ppt)") + guides(fill="none")
#SCPData %>% ggplot(aes(y=Count, x=Salinity))+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw()+ ylab("Count") + xlab("Salinity") #+ facet_wrap(~Year)
...
Plot_Water<-SCPData_Positive %>% mutate(water_temp_bin = as.factor(round(WaterTemperature)))  %>% mutate(water_temp_bin = as.numeric(water_temp_bin))  %>% 
  group_by(water_temp_bin, AgeClass) %>% summarize(TotalCount=sum(Count)) %>% 
  ggplot(aes(x=water_temp_bin, y=TotalCount, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs( x="Water Temperature (C)") + guides(fill="none")


Plot_DO<-SCPData_Positive %>% mutate(do_bin = as.factor(round(DO)))  %>% mutate(do_bin = as.numeric(do_bin))  %>%  group_by(do_bin, AgeClass) %>% summarize(TotalCount=sum(Count)) %>% 
  ggplot(aes(x=do_bin, y=TotalCount, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Dissolved Oxygen (mg/L)")


Plot_Secchi<-SCPData_Positive %>% group_by(Secchi, AgeClass) %>% summarize(TotalCount=sum(Count)) %>% 
  ggplot(aes(x=Secchi, y=TotalCount, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Secchi (cm)")+ guides(fill="none")


Plot_Salinity<-SCPData_Positive %>% mutate(salinity_bin = as.factor(round(Salinity)))  %>%  mutate(salinity_bin = as.numeric(salinity_bin)) %>%  group_by(salinity_bin, AgeClass) %>% summarize(TotalCount=sum(Count)) %>% 
  ggplot(aes(x=salinity_bin, y=TotalCount, fill=AgeClass))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Salinity (ppt)") + guides(fill="none")
...
Plot_Water<-PricklySculpinDataPA %>% ggplot(aes(y=PresAbs, x=WaterTemperature, fill=AgeClass)) + geom_violin()+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", position = position_dodge(width = 0.9), color="black") + theme_bw(base_size = 18) + ylab("") + xlab("Water Temperature (C)")+ guides(fill="none") + facet_wrap(~AgeClass, nrow=2)

Plot_DO<-PricklySculpinDataPA %>% ggplot(aes(y=PresAbs, x=DO, fill=AgeClass)) + 
  geom_violin()+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", position = position_dodge(width = 0.9), color="black") + theme_bw(base_size = 18)+ ylab("")+ xlab("Dissolved Oxygen (mg/L)") + facet_wrap(~AgeClass, nrow=2)

Plot_Secchi<-PricklySculpinDataPA %>% ggplot(aes(y=PresAbs, x=Secchi, fill=AgeClass)) + geom_violin()+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", position = position_dodge(width = 0.9), color="black") + theme_bw(base_size = 18)+ ylab("") + xlab("Secchi (cm)") + facet_wrap(~AgeClass, nrow=2)+ guides(fill="none") #+ facet_wrap(~Year)

Plot_Salinity<-PricklySculpinDataPA %>% ggplot(aes(y=PresAbs, x=Salinity, fill=AgeClass)) + geom_violin()+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", position = position_dodge(width = 0.9), color="black") + theme_bw(base_size = 18)+ ylab("") + xlab("Salinity (ppt)")+ guides(fill="none")+ facet_wrap(~AgeClass, nrow=2) #+ facet_wrap(~Year)
...
### Setting data and hyperparameters
```{r}
PricklySculpinAge0<-PricklySculpinData %>% filter(AgeClass %in% "Age-0")
str(PricklySculpinAge0)
unique(PricklySculpinAge0$Binary)
(sum(PricklySculpinAge0$Binary)/nrow(PricklySculpinAge0))*100

SummTable2 <- PricklySculpinAge0 %>% #Keep in mind that the ages are separate but included. A sampling event with both adult and age-0 fish count as present for 2 samples.
  group_by(PresAbs) %>% #break down yearly summaries by including Year as a grouping variable. 
  dplyr::summarize(
    Mean_WT = mean(WaterTemperature, na.rm=TRUE),
    SD_WT = sd(WaterTemperature, na.rm=TRUE),
...
#plots
dismo::gbm.plot(SCPbrt_Age0, n.plots = 4, write.title= F, rug = T, smooth = TRUE, plot.layout=c(2,2), common.scale = T, y.label = "Fitted Value") #Makes the partial dependence plots.
...
WaterTempDenisty<-ggplot(YOY_SCPData_Positive, aes(WaterTemperature))+geom_area(stat="bin")+ theme_bw()

SalinityDenisty<-ggplot(YOY_SCPData_Positive, aes(Salinity))+geom_area(stat="bin")+ theme_bw()

DODenisty<-ggplot(YOY_SCPData_Positive, aes(DO))+geom_area(stat="bin")+ theme_bw()

SecchiDenisty<-ggplot(YOY_SCPData_Positive, aes(Secchi))+geom_area(stat="bin")+ theme_bw()
...
### Setting data and hyperparameters
```{r}
PricklySculpinAge1plus<-PricklySculpinData %>% filter(AgeClass %in% "Age-1+")
str(PricklySculpinAge1plus)
unique(PricklySculpinAge1plus$AgeClass)
#gbm.step wants bernoulli response {0, 1} 

SummTable3 <- PricklySculpinAge1plus %>% #Keep in mind that the ages are separate but included. A sampling event with both adult and age-0 fish count as present for 2 samples.
  group_by(PresAbs) %>% #break down yearly summaries by including Year as a grouping variable. 
  dplyr::summarize(
    Mean_WT = mean(WaterTemperature, na.rm=TRUE),
    SD_WT = sd(WaterTemperature, na.rm=TRUE),
...
#plots
dismo::gbm.plot(SCPbrt_Age1P, n.plots = 4, write.title= F, rug = T, smooth = TRUE, plot.layout=c(2,2), common.scale = T, y.label = "Fitted Value") #Makes the partial dependence plots.
...
WaterTempDenisty<-ggplot(SCP1P_Positive, aes(WaterTemperature))+geom_area(stat="bin")+ theme_bw()

SalinityDenisty<-ggplot(SCP1P_Positive, aes(Salinity))+geom_area(stat="bin")+ theme_bw()

DODenisty<-ggplot(SCP1P_Positive, aes(DO))+geom_area(stat="bin")+ theme_bw()

SecchiDenisty<-ggplot(SCP1P_Positive, aes(Secchi))+geom_area(stat="bin")+ theme_bw()
