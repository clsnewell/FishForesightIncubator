...
sum(is.na(SMFS_OTR_Thesis$WaterTemperature)) #44
sum(is.na(SMFS_OTR_Thesis$DO)) #495
sum(is.na(SMFS_OTR_Thesis$Salinity)) #47
sum(is.na(SMFS_OTR_Thesis$Secchi)) #100
#This isn't super helpful... We have multiple rows for one 
Na_Assessment<-SMFS_OTR_Thesis %>% 
...
SMFS_OTR_Thesis_WQNA<-SMFS_OTR_Thesis %>% filter(SampleRowID %in% NaSamples) %>% group_by(SampleRowID) %>% select(SampleRowID, StationCode, MethodCode, TowDuration, WaterTemperature, Secchi, Salinity, DO, PctSaturation, SpecificConductance, TideCode, SampleDate, Year, WaterYear)
...
#Save New DF
write.csv(SMFS, "C:/Users/cnewe/OneDrive/Documents/Incubator/Code/FishForesightIncubator/Data/SMFS.csv")
```