...
PresAbs %>% mutate(Type= ifelse(is.na(FishID), "Plus", "Measured")) %>% filter(gensp %in% "Gasterosteus aculeatus") %>% ggplot(aes(x=StandardLength,y=Count, fill=Type)) + geom_bar(stat="identity")+ ggtitle("Length distribution of stickleback 2011-2023") +theme_bw(base_size = 20)+xlab("Standard length (mm)")
```
...
Plot_Salinity<-SticklebackData %>% ggplot(aes(y=PresAbs, x=Salinity)) + geom_violin(fill = "lightseagreen")+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw(base_size = 18)+ ylab("") + xlab("Salinity (ppt)") #+ facet_wrap(~Year)
...
# 4 panel of wq CPUE
Plot_Water<-SticklebackData %>% mutate(water_temp_bin = as.numeric( round(WaterTemperature)))  %>% 
...
Plot_DO<-SticklebackData %>% mutate(do_bin = as.numeric(round(DO)))  %>% group_by(do_bin) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
...
Plot_Secchi<-SticklebackData %>% group_by(Secchi) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ...
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Secchi (cm)")+ scale_x_continuous(limits=c(0,100))


Plot_Salinity<-SticklebackData %>% mutate(salinity_bin = as.numeric(round(Salinity)))  %>% group_by(salinity_bin) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
...
Plot_DO<-SticklebackData %>% mutate(do_bin = as.numeric(round(DO)))  %>% group_by(do_bin) %>% summarize(TotalCount=sum(Count)) %>% 
  ggplot(aes(x=do_bin, y=TotalCount))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Dissolved Oxygen (mg/L)")


Plot_Secchi<-SticklebackData %>% group_by(Secchi) %>% summarize(TotalCount=sum(Count)) %>% 
  ggplot(aes(x=Secchi, y=TotalCount))+
  geom_bar(stat="identity")+ 
  theme_bw(base_size = 18)+ labs(x="Secchi (cm)")+ scale_x_continuous(limits=c(0,100))


Plot_Salinity<-SticklebackData %>% mutate(salinity_bin = as.numeric(round(Salinity)))  %>% group_by(salinity_bin) %>% summarize(TotalCount=sum(Count)) %>% 
...
WaterTempDenisty<-ggplot(SticklebackData_PosCount, aes(WaterTemperature))+geom_area(stat="bin")+ theme_bw()

SalinityDenisty<-ggplot(SticklebackData_PosCount, aes(Salinity))+geom_area(stat="bin")+ theme_bw()

DODenisty<-ggplot(SticklebackData_PosCount, aes(DO))+geom_area(stat="bin")+ theme_bw()

SecchiDenisty<-ggplot(SticklebackData_PosCount, aes(Secchi))+geom_area(stat="bin")+ theme_bw()
