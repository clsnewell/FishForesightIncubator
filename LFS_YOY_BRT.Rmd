...
```{r}
LongfinSmeltYOYDataPA<-PresAbs %>% filter(gensp %in% "Spirinchus thaleichthys", AgeClass %in% "Age-0") %>% dplyr::select(-...1, -...2)
LongfinSmeltYOYDataPA<-as.data.frame(LongfinSmeltYOYDataPA)
unique(LongfinSmeltYOYDataPA$AgeClass)
```
...
brt1$cv.statistics #These are the most appropriate evaluation statistics. We calculate each statistic within each fold (at the identied optimal number of trees that is calculated on the mean change in predictive deviance over all folds), then present here the mean and standard error of those fold-based statistics.
