...
PresAbs_OneRowPerSample<-PresAbs %>% group_by(SampleRowID) %>% summarize(Year=Year, Month=Month, StationCode=StationCode, WaterTemperature=WaterTemperature, Salinity=Salinity, DO=DO, Secchi=Secchi) %>% distinct()
#need cowplot
Year_DO<-PresAbs_OneRowPerSample %>% ggplot(aes(x=Year, y=DO))+geom_boxplot(fill="grey")+ylab("DO (mg/L)")+theme_bw() +                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
Year_Sal<-PresAbs_OneRowPerSample %>% ggplot(aes(x=Year, y=Salinity))+geom_boxplot(fill="grey")+ylab("SAL (ppt)")+theme_bw()+                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
Year_Temp<-PresAbs_OneRowPerSample %>% ggplot(aes(x=Year, y=WaterTemperature))+geom_boxplot(fill="grey")+ylab("TEMP (degC)")+theme_bw()+                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
Year_SEC<- PresAbs_OneRowPerSample %>% ggplot(aes(x=Year, y=Secchi))+geom_boxplot(fill="grey")+ylab("SEC (cm)")+theme_bw()+                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")

Month_DO<-PresAbs_OneRowPerSample %>% ggplot(aes(x=Month, y=DO))+geom_boxplot(fill="grey")+ylab("DO (mg/L)")+theme_bw() +                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
Month_Sal<-PresAbs_OneRowPerSample %>% ggplot(aes(x=Month, y=Salinity))+geom_boxplot(fill="grey")+ylab("SAL (ppt)")+theme_bw()+                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
Month_Temp<-PresAbs_OneRowPerSample %>% ggplot(aes(x=Month, y=WaterTemperature))+geom_boxplot(fill="grey")+ylab("TEMP (degC)")+theme_bw()+                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
Month_SEC<- PresAbs_OneRowPerSample %>% ggplot(aes(x=Month, y=Secchi))+geom_boxplot(fill="grey")+ylab("SEC (cm)")+theme_bw()+                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
sort(unique(PresAbs_OneRowPerSample$StationCode)) #gives list in alphabetical order!
#PresAbs_OneRowPerSample$StationCode<-as.factor(PresAbs_OneRowPerSample$StationCode, levels=c("GY1", "GY2", "GY3", "SU4", "SU3", "SU2", "SU1", "BY"))

StationCode_DO<-PresAbs_OneRowPerSample %>%  ggplot(aes(x=StationCode, y=DO))+geom_boxplot(fill="grey")+ylab("DO (mg/L)")+theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
StationCode_Sal<-PresAbs_OneRowPerSample%>% ggplot(aes(x=StationCode, y=Salinity))+geom_boxplot(fill="grey")+ylab("SAL (ppt)")+theme_bw()+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
StationCode_Temp<-PresAbs_OneRowPerSample %>% ggplot(aes(x=StationCode, y=WaterTemperature))+geom_boxplot(fill="grey")+ylab("TEMP (degC)")+theme_bw()+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
StationCode_SEC<- PresAbs_OneRowPerSample %>% ggplot(aes(x=StationCode, y=Secchi))+geom_boxplot(fill="grey")+ylab("SEC (cm)")+theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+                      # Add median line to boxplot
  stat_summary(fun = median,
               geom = "line",
               aes(group = 1),
               col = "red")
