...
### Summaries of catch
```{r}
PricklySculpinDataPA %>% group_by(Year) %>% dplyr::summarize(N=n(), Count = sum(Count), BinarySum = sum(Binary), Probability=(BinarySum/N)*100) #Boom and bust years, not great for loo validation.
...
(sum(PricklySculpinDataPA$Binary)/nrow(PricklySculpinDataPA))*100
```

13.32% probability of capturing PricklySculpin.

### Summaries of samples
```{r}
range(PricklySculpinDataPA$WaterTemperature)
mean(PricklySculpinDataPA$WaterTemperature)

range(PricklySculpinDataPA$DO)
mean(PricklySculpinDataPA$DO)

range(PricklySculpinDataPA$Salinity)
mean(PricklySculpinDataPA$Salinity)

range(PricklySculpinDataPA$Secchi)
mean(PricklySculpinDataPA$Secchi)
...
calc.deviance(PricklySculpinDataPA_Test$Binary, Preds, calc.mean=T, family="bernoulli") #5.5702

roc(PricklySculpinDataPA_Test$Binary, Preds) #Area under the curve: 0.7681
