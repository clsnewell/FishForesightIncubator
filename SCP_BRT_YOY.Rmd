...
```{r}
PricklySculpinYOYData<-PresAbs %>% filter(gensp %in% "Cottus asper", AgeClass %in% "Age-0") %>% dplyr::select(-...1, -...2)
PricklySculpinYOYData<-as.data.frame(PricklySculpinYOYData)
unique(PricklySculpinYOYData$AgeClass)
```

### Ages
```{r}
PricklySculpinYOYData %>% group_by(AgeClass)%>% dplyr::summarize(N=n(), Count = sum(Count), BinarySum = sum(Binary), Probability=(BinarySum/N)*100)# %>% print(n=26)
```
Now lets check the dataset.

```{r}
glimpse(PricklySculpinYOYData)
summary(PricklySculpinYOYData)
NumSamples<-unique(PricklySculpinYOYData$SampleRowID) #3505 sampling events.
```
...
```{r}
PricklySculpinYOYDataPA<-PricklySculpinYOYData %>% dplyr::group_by(SampleRowID, WaterTemperature, DO, Secchi, Salinity, Year, Month, StationCode) %>% dplyr::summarize(Count=sum(Count)) %>% mutate(PresAbs = if_else(Count > 0, "present", "absent"), Binary=if_else(Count > 0, 1, 0))

PricklySculpinYOYDataPA$PresAbs<-as.factor(PricklySculpinYOYDataPA$PresAbs)

PricklySculpinYOYDataPA$Year<-as.factor(PricklySculpinYOYDataPA$Year)

PricklySculpinYOYDataPA$Month<-as.factor(PricklySculpinYOYDataPA$Month)

PricklySculpinYOYDataPA$StationCode<-as.factor(PricklySculpinYOYDataPA$StationCode)

PricklySculpinYOYDataPA<-as.data.frame(PricklySculpinYOYDataPA)
```

### Summaries of catch
```{r}
PricklySculpinYOYDataPA %>% group_by(Year) %>% dplyr::summarize(N=n(), Count = sum(Count), BinarySum = sum(Binary), Probability=(BinarySum/N)*100) #Boom and bust years, not great for loo validation.

PricklySculpinYOYDataPA %>% group_by(StationCode) %>% dplyr::summarize(N=n(), Count = sum(Count), BinarySum = sum(Binary), Probability=(BinarySum/N)*100) %>% print(n=24) #Some are heavily favored.

PricklySculpinYOYDataPA %>% group_by(Month) %>% dplyr::summarize(N=n(), Count = sum(Count), BinarySum = sum(Binary), Probability=(BinarySum/N)*100) #Never in January and February

#Overall capture probability
OverallProb<-(sum(PricklySculpinYOYDataPA$Binary)/nrow(PricklySculpinYOYDataPA))*100
```
...
```{r}
RangeWT<-range(PricklySculpinYOYDataPA$WaterTemperature)
MeanWT<-mean(PricklySculpinYOYDataPA$WaterTemperature)

RangeDO<-range(PricklySculpinYOYDataPA$DO)
MeanDO<-mean(PricklySculpinYOYDataPA$DO)

RangeSal<-range(PricklySculpinYOYDataPA$Salinity)
MeanSal<-mean(PricklySculpinYOYDataPA$Salinity)

RangeSec<-range(PricklySculpinYOYDataPA$Secchi)
MeanSec<-mean(PricklySculpinYOYDataPA$Secchi)
...
Plot_Water<-PricklySculpinYOYData %>% ggplot(aes(y=PresAbs, x=WaterTemperature)) + geom_violin(fill = "lightseagreen")+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw() + ylab("") + xlab("Water Temperature (C)") #+ facet_wrap(~Year)

Plot_DO<-PricklySculpinYOYData %>% ggplot(aes(y=PresAbs, x=DO)) + 
...
Plot_Secchi<-PricklySculpinYOYData %>% ggplot(aes(y=PresAbs, x=Secchi)) + 
...
Plot_Salinity<-PricklySculpinYOYData %>% ggplot(aes(y=PresAbs, x=Salinity)) + geom_violin(fill = "lightseagreen")+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw()+ ylab("") + xlab("Salinity") #+ facet_wrap(~Year)

Plots<-plot_grid(Plot_Water, Plot_DO, Plot_Secchi, Plot_Salinity, ncol = 2, labels = "AUTO")

# now add the title
title <- ggdraw() + draw_label("YOY Prickly Sculpin presence/absence by water quality variables for SMFS 2011-2023", fontface = 'bold', x = 0, hjust = 0) +  theme(
...
SummTable <- PricklySculpinYOYData %>% 
...
print(SummTable)
write.csv(SummTable, here("data", "PricklySculpinYOY_SummTable.csv"))

#Seasonality
PricklySculpinYOYData %>% summarize(TotalPresence=sum(Binary), sd_binary=sd(Binary), n=n(), PercentageOfPresence=100*(TotalPresence/n))

PricklySculpinYOYData$Month<-as.factor(PricklySculpinYOYData$Month)

PricklySculpinYOYData %>% group_by(Month) %>% summarize(TotalPresence=sum(Binary), sd_binary=sd(Binary), n=n(), PercentageOfPresence=100*(TotalPresence/n)) %>% ggplot(aes(x=Month, y=TotalPresence)) +geom_col()+geom_linerange(aes(ymin=TotalPresence-sd_binary, ymax=TotalPresence+sd_binary))+theme_bw(base_size = 26)+ylab("Trawls that catch YOY Prickly Sculpin") +xlab("Month (summed 2011-2023)") #+ theme(axis.text.x = element_text(angle = 45, hjust=1))
PricklySculpinYOYData %>% group_by(StationCode) %>% summarize(TotalPresence=sum(Binary), sd_binary=sd(Binary), n=n(), PercentageOfPresence=100*(TotalPresence/n)) %>% ggplot(aes(x=StationCode, y=PercentageOfPresence)) +geom_col()+theme_bw(base_size = 26)+ylab("% of trawls that catch YOY Prickly Sculpin") +xlab("Station Code") + theme(axis.text.x = element_text(angle = 45, hjust=1))
```
...
str(PricklySculpinYOYDataPA)
unique(PricklySculpinYOYDataPA$Binary)
...
set.seed(124) # for reproducibility 
#gbm.step wants bernoulli response {0, 1} 
brt1 <- dismo::gbm.step(data=PricklySculpinYOYDataPA, 
...
brt_SDM_k <- eval_kfold_brt(dataInput = PricklySculpinYOYDataPA,
                        gbm.x = c(2,3,4,5),
...
brt_SDM_loo <- eval_loo_brt(pres = PricklySculpinYOYDataPA[which(PricklySculpinYOYDataPA$Binary == 1),],
                        abs = PricklySculpinYOYDataPA[which(PricklySculpinYOYDataPA$Binary == 0),],
                        gbm.x = c(2,3,4,5),
                        gbm.y=11, learning.rate = LearningRate,
                        bag.fraction = Bag, tree.complexity = Complexity)
brt_SDM_loo
...
```{r}
set.seed(124)
inTrain<-createDataPartition(y=PricklySculpinYOYDataPA$Binary, p=0.8, list=FALSE) #data is already limited so I decided to use 80% of data to train and will test on 20%. 
PricklySculpinYOYDataPA_Train<-PricklySculpinYOYDataPA[inTrain,]
PricklySculpinYOYDataPA_Test<-PricklySculpinYOYDataPA[-inTrain,]
```
...
brt2 <- dismo::gbm.step(data=PricklySculpinYOYDataPA_Train, 
                         gbm.x= c(2,3,4,5), # environmental variables. 2 = Water Temperature, 5 = salinity, 3 = DO, 4 = Secchi
                         gbm.y= 11, 
                         family = "bernoulli", 
...
Preds<-predict(object = brt2, newdata = PricklySculpinYOYDataPA_Test, type = "response")

#PredGrid<-gbm.predict.grids(brt2, PricklySculpinYOYDataPA_Test, want.grids=F, n.trees=brt2$gbm.call$best.trees, type="response")
PredBinaries<-as.factor(ifelse(Preds>0.5,1,0))

PricklySculpinYOYDataPA_Test$Binary<-as.factor(PricklySculpinYOYDataPA_Test$Binary)

confusionMatrix(PredBinaries, PricklySculpinYOYDataPA_Test$Binary)

PricklySculpinYOYDataPA_Test$Binary<-as.numeric(PricklySculpinYOYDataPA_Test$Binary)

calc.deviance(PricklySculpinYOYDataPA_Test$Binary, Preds, calc.mean=T, family="bernoulli") #3.599321

roc(PricklySculpinYOYDataPA_Test$Binary, Preds) #Area under the curve: 0.8104

ROC<-pROC::roc(PredBinaries, PricklySculpinYOYDataPA_Test$Binary)
auc<-round(auc(PredBinaries, PricklySculpinYOYDataPA_Test$Binary),4)
ggroc(ROC,colour = 'steelblue', size = 2) + #create ROC plot
  ggtitle(paste0('ROC Curve', '(AUC= ',auc,') '))+
  theme_minimal()
PredBinaries<-as.numeric(PredBinaries)
cor(PredBinaries, PricklySculpinYOYDataPA_Test$Binary)
plot(PredBinaries, PricklySculpinYOYDataPA_Test$Binary)
...
gbmFitKappa_split <- train(PresAbs ~ WaterTemperature + Secchi + DO + Salinity, #caret wants non-numeric presence, absence. Might need to change it to Yes and No.
                data = PricklySculpinYOYDataPA_Train, 
                method = "gbm", 
                trControl = fitControlKappa, 
                bag.fraction=Bag,
                na.action=na.pass,
...
Result<-predict(gbmFitKappa_split, PricklySculpinYOYDataPA_Test)

confusionMatrix(PricklySculpinYOYDataPA_Test$Binary, Result)
```
...
brt1 %>% partial(pred.var = c("WaterTemperature"),train=PricklySculpinYOYDataPA, n.trees=1800) %>% autoplot()
...
### Visualizations
```{r}
SCPYOYData<-PresAbs %>% filter(gensp %in% "Cottus asper", AgeClass %in% "Age-0") %>% dplyr::select(-...1, -...2)
SCPYOYData$Month<-as.factor(SCPYOYData$Month)

summary(SCPYOYData)

# I want to show presences vs. all samples.

Plot_Water<-SCPYOYData %>% mutate(water_temp_bin = as.factor( round(WaterTemperature * 2) / 2))  %>% 
  ...
  theme_bw()+ labs(title = "YOY prickly sculpin & water temperature SMFS 2011-2023", y ="Catch per minute trawl", x="Water Temperature (C)", caption = "Water temperatures rounded to 0.5 then grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin.") + theme(axis.text.x = element_text(angle = 45, hjust=1))
...
Plot_DO<-SCPYOYData %>% mutate(do_bin = as.factor(round(DO * 2) / 2))  %>% group_by(do_bin) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=do_bin, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "YOY prickly sculpin & dissolved oxygen SMFS 2011-2023", y ="Catch per minute trawl", x="Dissolved Oxygen (mg/L)", caption = "Dissolved oxygen rounded to 0.5 then grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")

#SCPYOYData %>% ggplot(aes(y=Count, x=DO)) + stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw()+ ylab("Count")+ xlab("Dissolved Oxygen (mg/L)") #+ facet_wrap(~Year)

Plot_Secchi<-SCPYOYData %>% group_by(Secchi) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=Secchi, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "YOY prickly sculpin & dissolved oxygen SMFS 2011-2023", y ="Catch per minute trawl", x="Secchi (cm)", caption = "Secchi grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")
#SCPYOYData %>% ggplot(aes(y=Count, x=Secchi)) + stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw()+ ylab("Count") + xlab("Secchi (cm)") #+ facet_wrap(~Year)

Plot_Salinity<-SCPYOYData %>% mutate(salinity_bin = as.factor(round(Salinity * 2) / 2))  %>% group_by(salinity_bin) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=salinity_bin, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "YOY prickly sculpin & dissolved oxygen SMFS 2011-2023", y ="Catch per minute trawl", x="Salinity", caption = "Salinity  rounded to 0.5 then grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")
#SCPYOYData %>% ggplot(aes(y=Count, x=Salinity))+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw()+ ylab("Count") + xlab("Salinity") #+ facet_wrap(~Year)

Plots<-plot_grid(Plot_Water, Plot_DO, Plot_Secchi, Plot_Salinity, ncol = 2, labels = "AUTO")

# now add the title
title <- ggdraw() + draw_label("Age-0 prickly sculpin count by water quality variables for SMFS 2011-2023", fontface = 'bold', x = 0, hjust = 0) +  theme(
    # add margin on the left of the drawing canvas,
    # so title is aligned with left edge of first plot
    plot.margin = margin(0, 0, 0, 7))
...
#Seasonality
SCPYOYData %>% dplyr::summarize(TotalCount=sum(Count), sd_Count=sd(Count), n=n(), PercentageOfPresence=100*(TotalCount/n)) #Total presence = 1971 of 5471. 36.03% presence of samples.
SCPYOYData %>%  group_by(Month) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=Month, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "YOY prickly sculpin by month SMFS 2011-2023", y ="Catch per minute trawl", x="Month", caption = "Month grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")
...
SCPYOYData %>% group_by(StationCode) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=StationCode, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "YOY prickly sculpin by station SMFS 2011-2023", y ="Catch per minute trawl", x="Station Code", caption = "Station code grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")+ theme(axis.text.x = element_text(angle = 45, hjust=1))
...
```{r}
SCPYOYData_All<-PresAbs %>% filter(gensp %in% "Cottus asper", AgeClass %in% "Age-0") %>% dplyr::select(-...1, -...2) %>% group_by(SampleRowID, WaterTemperature, Salinity, Secchi, DO) %>% summarize(Count=sum(Count))
...
### gbm.step
```{r}
system.time( brt3 <- dismo::gbm.step(data=SCPYOYData_All, 
...
```{r}
SCPYOYData_Positive<-PresAbs %>% filter(gensp %in% "Cottus asper", AgeClass %in% "Age-0", Binary == 1) %>% group_by(SampleRowID, WaterTemperature, Salinity, Secchi, DO) %>% summarize(Count=sum(Count))
SCPYOYData_Positive<-as.data.frame(SCPYOYData_Positive)
summary(SCPYOYData_Positive)
```
...
### gbm.step
```{r}
system.time( brt4 <- dismo::gbm.step(data=SCPYOYData_Positive, 
                         gbm.x= c(2,3,4,5), # environmental variables.
                         gbm.y= 6, # Binary response variable
                         family = "gaussian", # for counts this would be "poisson"
                         tree.complexity = Complexity, # complexity of the interactions that the model will fit. 5-way interaction since these may all interact with one another? Typical values range from a depth of 3–8 but it is not uncommon to see a tree depth of 1 (J. Friedman, Hastie, and Tibshirani 2001). Smaller depth trees such as decision stumps are computationally efficient (but require more trees); however, higher depth trees allow the algorithm to capture unique interactions but also increase the risk of over-fitting. Note that larger  n  or  p training data sets are more tolerable to deeper trees.
...
```{r}
set.seed(124)
inTrain<-createDataPartition(y=SCPYOYData_Positive$Count, p=0.8, list=FALSE) #data is already limited so I decided to use 80% of data to train and will test on 20%. 
LongfinSmeltYOYData_Train<-SCPYOYData_Positive[inTrain,]
LongfinSmeltYOYData_Test<-SCPYOYData_Positive[-inTrain,]
```
...
brt_SDM_k <- eval_kfold_brt(dataInput = SCPYOYData_Positive,
                        gbm.x = c(2,3,4,5),
...
brt_SDM_loo <- eval_loo_brt(pres = SCPYOYData_Positive[which(SCPYOYData_Positive$Binary == 1),],
                        abs = SCPYOYData_Positive[which(SCPYOYData_Positive$Binary == 0),],
                        gbm.x = c(2,3,4,5),
                        gbm.y=6, learning.rate = LearningRate,
                        bag.fraction = Bag, tree.complexity = Complexity)
brt_SDM_loo
