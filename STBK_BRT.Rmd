...
```{r}
SticklebackData<-PresAbs %>% filter(gensp %in% "Gasterosteus aculeatus")
...
```{r}
Plot_Water<-SticklebackData %>% ggplot(aes(y=Binary, x=WaterTemperature, color = TowDuration)) + 
  geom_point()+ theme_bw()

Plot_DO<-SticklebackData %>% ggplot(aes(y=Binary, x=DO, color = TowDuration)) + 
  geom_point()+ theme_bw()

Plot_Secchi<-SticklebackData %>% ggplot(aes(y=Binary, x=Secchi, color = TowDuration)) + 
  geom_point()+ theme_bw()

Plot_Salinity<-SticklebackData %>% ggplot(aes(y=Binary, x=Salinity, color = TowDuration)) + 
  geom_point() + theme_bw()
...
Plot_Water<-SticklebackData %>% ggplot(aes(y=PresAbs, x=WaterTemperature, color = TowDuration)) + geom_violin(fill = "lightseagreen")+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw() + ylab("") + xlab("Water Temperature (C)") #+ facet_wrap(~Year)

Plot_DO<-SticklebackData %>% ggplot(aes(y=PresAbs, x=DO, color = TowDuration)) + 
...
Plot_Secchi<-SticklebackData %>% ggplot(aes(y=PresAbs, x=Secchi, color = TowDuration)) + 
...
Plot_Salinity<-SticklebackData %>% ggplot(aes(y=PresAb, x=Salinity, color = TowDuration)) + geom_violin(fill = "lightseagreen")+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw()+ ylab("") + xlab("Salinity") #+ facet_wrap(~Year)
...
#gbm.step wants bernoulli response {0, 1} 
 brt1 <- dismo::gbm.step(data=SticklebackData, 
...
Stickles<-PresAbs %>% filter(gensp %in% "Gasterosteus aculeatus")
```

### Visualizations
```{r}
summary(SticklebackDataAllCounts)

# I want to show presences vs. all samples.

Plot_Water<-Stickles %>% mutate(water_temp_bin = as.factor( round(WaterTemperature * 2) / 2))  %>% 
  ...
  theme_bw()+ labs(title = "Stickleback & water temperature SMFS 2011-2023", y ="Catch per minute trawl", x="Water Temperature (C)", caption = "Water temperatures rounded to 0.5 then grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin.") + theme(axis.text.x = element_text(angle = 45, hjust=1))
...
Plot_DO<-Stickles %>% mutate(do_bin = as.factor(round(DO * 2) / 2))  %>% group_by(do_bin) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=do_bin, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "Stickleback & dissolved oxygen SMFS 2011-2023", y ="Catch per minute trawl", x="Dissolved Oxygen (mg/L)", caption = "Dissolved oxygen rounded to 0.5 then grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")

#Stickles %>% ggplot(aes(y=Count, x=DO)) + stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw()+ ylab("Count")+ xlab("Dissolved Oxygen (mg/L)") #+ facet_wrap(~Year)

Plot_Secchi<-Stickles %>% group_by(Secchi) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=Secchi, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "Stickleback & dissolved oxygen SMFS 2011-2023", y ="Catch per minute trawl", x="Secchi (cm)", caption = "Secchi grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")
#Stickles %>% ggplot(aes(y=Count, x=Secchi)) + stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw()+ ylab("Count") + xlab("Secchi (cm)") #+ facet_wrap(~Year)

Plot_Salinity<-Stickles %>% mutate(salinity_bin = as.factor(round(Salinity * 2) / 2))  %>% group_by(salinity_bin) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=salinity_bin, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "Stickleback & dissolved oxygen SMFS 2011-2023", y ="Catch per minute trawl", x="Salinity", caption = "Salinity  rounded to 0.5 then grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")
...
#Seasonality
Stickles %>% dplyr::summarize(TotalCount=sum(Count), sd_Count=sd(Count), n=n(), PercentageOfPresence=100*(TotalCount/n)) #Total presence = 1971 of 5471. 36.03% presence of samples.
Stickles$Month<-as.factor(Stickles$Month)
Stickles %>%  group_by(Month) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=Month, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "Stickleback by month SMFS 2011-2023", y ="Catch per minute trawl", x="Month", caption = "Month grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")

Stickles %>% group_by(StationCode) %>% summarize(TotalCount=sum(Count), TotalTow=sum(TowDuration), CPUE = TotalCount/TotalTow) %>% 
  ggplot(aes(x=StationCode, y=CPUE))+
  geom_bar(stat="identity")+ 
  theme_bw()+ labs(title = "Stickleback by Station Code SMFS 2011-2023", y ="Catch per minute trawl", x="Station Code", caption = "Station code grouped to get sum of counts and sum trawl duration to get CPUE of counts by bin divided by total tow minutes for that bin")+ theme(axis.text.x = element_text(angle = 45, hjust=1))
  
```

## Data filtering for all counts
```{r}
SticklebackDataAllCounts<-PresAbs %>% filter(gensp %in% "Gasterosteus aculeatus") %>% group_by(SampleRowID, WaterTemperature, Salinity, Secchi, DO) %>% summarize(Count=sum(Count))
SticklebackDataAllCounts<-as.data.frame(SticklebackDataAllCounts)
```
...
### gbm.step
```{r}
system.time( brt3 <- dismo::gbm.step(data=SticklebackDataAllCounts, 
                         gbm.x= c(2,3,4,5), # environmental variables.
                         gbm.y= 6, # count response variable
...
#5 is tow duration. 4 is secchi. 1 is water temp.2 is salinity. 3 is DO
# save the model
saveRDS(brt3, here("data", "Stickleback_count_brt_gbm_step.rds"))
```


## Data filtering for above 0 counts
```{r}
Stickleback_Positive<-PresAbs %>% filter(gensp %in% "Gasterosteus aculeatus", Binary == 1) %>% group_by(SampleRowID, WaterTemperature, Salinity, Secchi, DO) %>% summarize(Count=sum(Count)) 
Stickleback_Positive<-as.data.frame(Stickleback_Positive)
summary(Stickleback_Positive)
```
...
### gbm.step
```{r}
system.time( brt4 <- dismo::gbm.step(data=Stickleback_Positive, 
                         gbm.x= c(2,3,4,5), # environmental variables.
...
mean(brt4$residuals^2) #mean residual deviance 0.309
#brt1$
names(brt4) #tells us the components of output
CV_deviance_explained <- (((brt4$self.statistics$mean.null- brt4$cv.statistics$deviance.mean)/brt4$self.statistics$mean.null)*100) 

CV_Correlation<-brt4$cv.statistics$correlation.mean

# To pull out one component of the list, use a number (angaus.tc5.lr01[[29]]) or name (angaus.tc5.lr01$cv.statistics); but be careful - some are as big as the dataset! e.g. there will be 1000 fitted values - find this by typing length(angaus.tc5.lr01$fitted)


 # reponse curves
dismo::gbm.plot(brt4, n.plots = 4, write.title= F, rug = T, smooth = TRUE, plot.layout=c(2,2), common.scale = T, y.label = "Fitted Value") #Makes the partial dependence plots.

gbm::plot.gbm(brt4, n.plots=4, return.grid=TRUE)

dismo::gbm.plot.fits(brt4)
#I assume these wtm's are weighted means and are the means used when calculating partial dependence? So each dot is a prediction made from a single tree?


# This code assesses the extent to which pairwise interactions exist in the data.

find.int <- gbm.interactions(brt4)
find.int#Salinity and DO, Salinity and Water Temp.
brt4$self.statistics$correlation[[1]] #The relevant set of evaluation statistics, calculated on the fitted values - i.e. this is only interesting in so far as it demonstrates "evaluation" (i.e. t) on the training data. It should NOT be reported as the model predictive performance.
# The returned object, here named test.int, is a list. The first 2 components summarise the results, first as a ranked list of the 5 most important pairwise interactions, and the second tabulating all pairwise interactions. The variable index numbers in $rank.list can be used for plotting.

brt4$cv.statistics #These are the most appropriate evaluation statistics. We calculate each statistic within each fold (at the identifed optimal number of trees that is calculated on the mean change in predictive deviance over all folds), then present here the mean and standard error of those fold-based statistics.

# You can plot pairwise interactions like this:
par(mfrow=c(1,2))
gbm.perspec(brt4,2, 4) #3D PLOT of Water Temp and Salinity
gbm.perspec(brt4,1, 4) #salinity and DO
gbm.perspec(brt4,1, 2)
#5 is tow duration. 4 is secchi. 1 is water temp.2 is salinity. 3 is DO
# save the model
saveRDS(brt4, here("data", "Stickleback_positivecount_brt_gbm_step.rds"))
```
...
# get the matrix out of the `plot.gbm`
var<-brt3$brt$var.names
response_data<-data.frame()

for (i in 1:length(var)){
response_plot_data <- gbm::plot.gbm(brt3$brt,
                                 i.var = var[i],
                                 return.grid = TRUE)
