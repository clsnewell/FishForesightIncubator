...
SummPlusCounts<-PlusCounts %>% group_by(CommonName) %>% summarize(nrows= n(), CountTotal = sum(Count))
...
negative_sl_rows <- PlusAssigned[PlusAssigned$sl < 0, ]
...
#Use this list of SampleRowIds to look at the fish size distributions for that same day...

ViewDist<-SMFS_Integrate %>% filter(SampleRowID %in% "{001EFF22-FE47-4468-92F6-6F1531090921}", gensp %in% "Morone saxatilis")

for(i in Unique_slrows) 
{
  SMFS_Integrate %>% filter(SampleRowID %in% i) 
}
```
