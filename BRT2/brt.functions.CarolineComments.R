"gbm.step" <-function (data,                             # gbm.step is a function that idk
  gbm.x,                                    # the predictors
  gbm.y,                                    # and response
  offset = NULL,                            # allows an offset to be specified. Idk what offset means.
  fold.vector = NULL,                       # allows a fold vector to be read in for CV with offsets, also unclear!
  tree.complexity = 1,                      # sets the complexity of individual trees. Tree complexity refers to number of splits? Which refers to amount of interaction between parameters...
  learning.rate = 0.01,                     # sets the weight applied to individudal trees. Changes the error included in the update rule, set between 0 and 1. Smaller values lead to slower learning rate, which needs to be compensated for with a proportional increase in iterations. Slowing the learning rate is a form of shrinkage that penalizes the sum of the absolute size of the estimates of Bm (estimated by min of loss function).
  bag.fraction = 0.75,                      # sets the proportion of observations used in selecting variables, I think referring to the splitting of data into test and training?
  ...
  prev.stratify = TRUE,                     # prevalence stratify the folds - only for p/a data. Im not using this, though would be interesting to compare results...
  family = "bernoulli",                     # family - bernoulli (=binomial), poisson, laplace or gaussian. How to decide which to use? Poisson probably since we have count data...
...
"gbm.fixed" <-function (data,                        # the input dataframe
...
"gbm.holdout" <-function (data,                        # the input data frame
...
"gbm.plot" <- function(gbm.object,                # a gbm object - could be one from gbm.step
...
"gbm.simplify" <- function(gbm.object,          # a gbm object describing sample intensity 
