...
### 1 row for each sampling event with age classes.
```{r}
PricklySculpinDataPA<-PricklySculpinData %>% dplyr::group_by(SampleRowID, WaterTemperature, DO, Secchi, Salinity, Year, Month, StationCode, AgeClass) %>% dplyr::summarize(Count=sum(Count)) %>% mutate(PresAbs = if_else(Count > 0, "present", "absent"), Binary=if_else(Count > 0, 1, 0))
...
### Summaries of catch
```{r}
PricklySculpinDataPA %>% group_by(AgeClass) %>% dplyr::summarize(N=n(), Count = sum(Count), BinarySum = sum(Binary), Probability=(BinarySum/N)*100) #Boom and bust years, not great for loo validation.

print(PricklySculpinDataPA %>% group_by(AgeClass,Year) %>% dplyr::summarize(N=n(), Count = sum(Count), BinarySum = sum(Binary), Probability=(BinarySum/N)*100), n=26) #Boom and bust years, not great for loo validation.
...
### 1 row for each sampling event without age classes.
```{r}
PricklySculpinDataPA<-PricklySculpinData %>% dplyr::group_by(SampleRowID, WaterTemperature, DO, Secchi, Salinity, Year, Month, StationCode) %>% dplyr::summarize(Count=sum(Count)) %>% mutate(PresAbs = if_else(Count > 0, "present", "absent"), Binary=if_else(Count > 0, 1, 0))
...
Plot_Water<-PricklySculpinData %>% ggplot(aes(y=PresAbs, x=WaterTemperature)) + geom_violin(fill = "lightseagreen")+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw() + ylab("") + xlab("Water Temperature (C)") #+ facet_wrap(~Year)

Plot_DO<-PricklySculpinData %>% ggplot(aes(y=PresAbs, x=DO)) + 
...
Plot_Secchi<-PricklySculpinData %>% ggplot(aes(y=PresAbs, x=Secchi)) + 
...
Plot_Salinity<-PricklySculpinData %>% ggplot(aes(y=PresAbs, x=Salinity)) + geom_violin(fill = "lightseagreen")+ stat_summary(fun.data="mean_sdl", fun.args=list(mult=1), geom= "pointrange", color="black") + theme_bw()+ ylab("") + xlab("Salinity") #+ facet_wrap(~Year)
...
#Seasonality
PricklySculpinData %>% summarize(TotalPresence=sum(Binary), sd_binary=sd(Binary), n=n(), PercentageOfPresence=100*(TotalPresence/n))

PricklySculpinData$Month<-as.factor(PricklySculpinData$Month)

PricklySculpinData %>% group_by(Month) %>% summarize(TotalPresence=sum(Binary), sd_binary=sd(Binary), n=n(), PercentageOfPresence=100*(TotalPresence/n)) %>% ggplot(aes(x=Month, y=TotalPresence)) +geom_col()+geom_linerange(aes(ymin=TotalPresence-sd_binary, ymax=TotalPresence+sd_binary))+theme_bw(base_size = 26)+ylab("Trawls that catch PricklySculpin") +xlab("Month (summed 2011-2023)") #+ theme(axis.text.x = element_text(angle = 45, hjust=1))
PricklySculpinData %>% group_by(StationCode) %>% summarize(TotalPresence=sum(Binary), sd_binary=sd(Binary), n=n(), PercentageOfPresence=100*(TotalPresence/n)) %>% ggplot(aes(x=StationCode, y=PercentageOfPresence)) +geom_col()+theme_bw(base_size = 26)+ylab("% of trawls that catch PricklySculpin") +xlab("Station Code") + theme(axis.text.x = element_text(angle = 45, hjust=1))
```
...
str(PricklySculpinDataPA)
unique(PricklySculpinDataPA$Binary)
...
set.seed(124) # for reproducibility 
#gbm.step wants bernoulli response {0, 1} 
brt1 <- dismo::gbm.step(data=PricklySculpinDataPA, 
...
gbm.perspec(brt1,1, 2) #Water temp and salinity
...
brt_SDM_k <- eval_kfold_brt(dataInput = PricklySculpinDataPA,
                        gbm.x = c(2,3,4,5),
...
brt_SDM_loo <- eval_loo_brt(pres = PricklySculpinDataPA[which(PricklySculpinDataPA$Binary == 1),],
                        abs = PricklySculpinDataPA[which(PricklySculpinDataPA$Binary == 0),],
                        gbm.x = c(2,3,4,5),
                        gbm.y=11, learning.rate = LearningRate,
                        bag.fraction = Bag, tree.complexity = Complexity)
brt_SDM_loo
...
```{r}
set.seed(124)
inTrain<-createDataPartition(y=PricklySculpinDataPA$Binary, p=0.8, list=FALSE) #data is already limited so I decided to use 80% of data to train and will test on 20%. 
PricklySculpinDataPA_Train<-PricklySculpinDataPA[inTrain,]
PricklySculpinDataPA_Test<-PricklySculpinDataPA[-inTrain,]
```
...
brt2 <- dismo::gbm.step(data=PricklySculpinDataPA_Train, 
                         gbm.x= c(2,3,4,5), # environmental variables. 2 = Water Temperature, 5 = salinity, 3 = DO, 4 = Secchi
                         gbm.y= 11, 
                         family = "bernoulli", 
...
Preds<-predict(object = brt2, newdata = PricklySculpinDataPA_Test, type = "response")

#PredGrid<-gbm.predict.grids(brt2, PricklySculpinDataPA_Test, want.grids=F, n.trees=brt2$gbm.call$best.trees, type="response")
PredBinaries<-as.factor(ifelse(Preds>0.5,1,0))

PricklySculpinDataPA_Test$Binary<-as.factor(PricklySculpinDataPA_Test$Binary)

confusionMatrix(PredBinaries, PricklySculpinDataPA_Test$Binary)

PricklySculpinDataPA_Test$Binary<-as.numeric(PricklySculpinDataPA_Test$Binary)

calc.deviance(PricklySculpinDataPA_Test$Binary, Preds, calc.mean=T, family="bernoulli") #3.599321

roc(PricklySculpinDataPA_Test$Binary, Preds) #Area under the curve: 0.8104

ROC<-pROC::roc(PredBinaries, PricklySculpinDataPA_Test$Binary)
auc<-round(auc(PredBinaries, PricklySculpinDataPA_Test$Binary),4)
ggroc(ROC,colour = 'steelblue', size = 2) + #create ROC plot
  ggtitle(paste0('ROC Curve', '(AUC= ',auc,') '))+
  theme_minimal()
PredBinaries<-as.numeric(PredBinaries)
cor(PredBinaries, PricklySculpinDataPA_Test$Binary)
plot(PredBinaries, PricklySculpinDataPA_Test$Binary)
```
...
brt_PricklySculpin <- here("data","PricklySculpin_brt_SDM_eval_gbm_step.rds") %>% readRDS()


# get the matrix out of the `plot.gbm`
var<-brt_PricklySculpin$brt$var.names
response_data<-data.frame()

for (i in 1:length(var)){
response_plot_data <- gbm::plot.gbm(brt_PricklySculpin$brt,
                                 i.var = var[i],
                                 return.grid = TRUE)
...
rel.inf<-data.frame(Variable = brt_PricklySculpin$brt$contributions$var,
                        rel.inf=brt_PricklySculpin$brt$contributions$rel.inf)# %>% 
