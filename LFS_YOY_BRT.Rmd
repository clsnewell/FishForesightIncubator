...
```{r}
set.seed(124)
inTrain<-createDataPartition(y=LongfinSmeltYOYData_All$Count, p=0.8, list=FALSE) #data is already limited so I decided to use 80% of data to train and will test on 20%. 
LongfinSmeltYOYData_Train<-LongfinSmeltYOYData_All[inTrain,]
LongfinSmeltYOYData_Test<-LongfinSmeltYOYData_All[-inTrain,]
```

### Running model again

```{r}
LearningRate<-0.01 #had to shift learning rate since only model 80% of the data.
brt4 <- dismo::gbm.step(data=LongfinSmeltYOYData_Train, 
...
Preds<-predict(object = brt4, newdata = LongfinSmeltYOYData_Test, type = "response")

#PredGrid<-gbm.predict.grids(brt2, LongfinSmeltYOYData_Test, want.grids=F, n.trees=brt2$gbm.call$best.trees, type="response")
PredBinaries<-as.factor(ifelse(Preds>0.1,1,0))

LongfinSmeltYOYData_Test$Binary<-as.factor(LongfinSmeltYOYData_Test$Binary)

confusionMatrix(PredBinaries, LongfinSmeltYOYData_Test$Binary)

LongfinSmeltYOYData_Test$Binary<-as.numeric(LongfinSmeltYOYData_Test$Binary)

calc.deviance(LongfinSmeltYOYData_Test$Binary, Preds, calc.mean=T, family="bernoulli") #6.890836
library(gbm)

roc(LongfinSmeltYOYData_Test$Binary, Preds) #Area under the curve: 0.8112

ROC<-pROC::roc(PredBinaries, LongfinSmeltYOYData_Test$Binary)
auc<-round(auc(PredBinaries, LongfinSmeltYOYData_Test$Binary),4)
ggroc(ROC,colour = 'steelblue', size = 2) + #create ROC plot
  ggtitle(paste0('ROC Curve', '(AUC= ',auc,') '))+
  theme_minimal()
PredBinaries<-as.numeric(PredBinaries)
cor(PredBinaries, LongfinSmeltYOYData_Test$Binary)
plot(PredBinaries, LongfinSmeltYOYData_Test$Binary)
```
Roc curve guide: https://www.statology.org/roc-curve-ggplot2/
...
#5 is tow duration. 4 is secchi. 1 is water temp.2 is salinity. 3 is DO
# save the model
saveRDS(brt_pcount, here("data", "LongfinSmelt_count_brt_gbm_step.rds"))
```
...
```{r}
set.seed(124)
inTrain<-createDataPartition(y=LongfinSmeltYOYData_Positive$Count, p=0.8, list=FALSE) #data is already limited so I decided to use 80% of data to train and will test on 20%. 
LongfinSmeltYOYData_Train<-LongfinSmeltYOYData_Positive[inTrain,]
LongfinSmeltYOYData_Test<-LongfinSmeltYOYData_Positive[-inTrain,]
```
...
brt4 <- dismo::gbm.step(data=LongfinSmeltYOYDataPA_Train, 
                         gbm.x= c(5,6,7,9), # environmental variables. 2 = Water Temperature, 5 = salinity, 3 = DO, 4 = Secchi
                         gbm.y= 22, 
                         family = "poisson", 
                         tree.complexity = Complexity,
