...
# Update the 'Class' column in 'Catch'
Raw_AgeClass$AgeClass <- ifelse(condition, Raw_AgeClass$AgeClass, "Delete")
unique(Raw_AgeClass$AgeClass)

Raw_AgeClass <- filter(Raw_AgeClass, AgeClass != "Delete")
...
```
I'm 7 rows off from the raw data... What is happening with them?

## Distribution of lengths for each water quality by year
...
WT.Plot<-Raw %>% 
  ...
  ggplot(aes(WaterTemperature, Count))+
  geom_col(aes(fill=StandardLength))+
  labs(title = "Longfin smelt standard lengths\n & Water Temp SMFS 2011-2023") +
  facet_wrap(~Year)+
  theme_bw()
WT.Plot
